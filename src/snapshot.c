    ...
    *current_out = -1;
    ...
    for (int si = 0; si < n; si++) {
        ...
        while (plugin_entry && snap->plugin_count < PB_MAX_PLUGINS) {
            ...
            /* Parameters (preferred over "ports" for mod-ui compat) */
            cJSON *params = cJSON_GetObjectItem(plugin_entry, "parameters");
            if (!cJSON_IsObject(params))
                params = cJSON_GetObjectItem(plugin_entry, "ports");
    ...
    for (int si = 0; si < count; si++) {
        ...
        for (int pi = 0; pi < snap->plugin_count; pi++) {
            ...
            for (int i = 0; i < sp->param_count; i++) {
                cJSON_AddNumberToObject(params_obj, sp->params[i].symbol,
            ...
            if (sp->preset_uri[0])
                cJSON_AddStringToObject(plugin_obj, "preset", sp->preset_uri);
